...
(function () {
  ...
  window.fcrIvrModal = function () {
    ...
    if (!sel.length) {
      if (typeof window.toast === 'function') window.toast('Select at least one candidate', 'ℹ️');
      return;
    }
    var jobs = callableJobs();

    window.fcrModal(
      '<div class="fcr-modal-head"><h3>AI Calling — ' + sel.length + ' selected</h3>' +
    ...
    api.get('/ai-calling/status').then(function (st) {
      ...
      body.innerHTML =
        ...
        '<div style="margin-top:10px">' + sel.map(function (c) {
        ...
        '<div style="display:flex;gap:8px;margin-top:14px;flex-wrap:wrap">' +
          '<button class="btn btn-primary" onclick="TL.calling.campaign()">🤖 Start AI Calling Campaign (' + sel.length + ')</button>' +
  ...
  /** Call everybody who is selected, for one requirement. */
